...
int ModuleRender::LoadFont(const char* texture_path, const char* characters, uint rows, uint columns)
{
	int id = -1;
...
void ModuleRender::BlitText(int x, int y, int font_id, const char* text, float size) const
{
	...
	for (uint i = 0; i < len; ++i)
	{
		...
		spriteRect.x = font->char_w * (charIndex % font->columns);
		spriteRect.y = font->char_h * (charIndex / font->columns);

		App->renderer->Blit(font->texture, x, y, &spriteRect, size, 0.0f, false);
